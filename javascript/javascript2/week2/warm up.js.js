console.log("===Home work js2 week3===");

console.log("___Warmup array exercises___");
...
console.log("---Working with movies---");


const movies = [{"title":"A Midsummer Night's Dream","year":1999,"rating":6.5,"votes":22260,"running_times":6960}
...
console.log(movies.filter( x => x.year <= 1989 && x.year >= 1980).length);


console.log("---step 2---");


let moviesWithTag = movies;
moviesWithTag.forEach(function (x) {
  return x.rating >= 7 ? x.tag = "good" : x.rating >= 4 && x.rating < 7 ? x.tag = "average" : x.tag = "bad";});
...
// function filtring(x,b) {
//   return x.filter(x=>x.b>6);
// };
console.log("can't be done with a constant function");
// function maping() {
//
// }
// movies = filtring(movies,"rating");
// console.log(movies);

// movies.filter(function filtring(x) {
//   return x.rating>6? x;});
// console.log(movies);
...
let arr = ["The", "dog", "who", "is", "not", "a", "man"];
for (var i = 0; i < arr.length; i++) {
  console.log(movies.indexOf(arr[i]));
}
